...
const app = (0, app_1.initializeApp)(appSettings);
const database = (0, database_1.getDatabase)(app);
const listItemsInDB = (0, database_1.ref)(database, "listItems");
...
addListButtonEl.addEventListener("click", function (event) {
    ...
    if (inputValue.length > 0 && inputValue.length <= 15) {
        (0, database_1.push)(listItemsInDB, inputValue);
    }
...
(0, database_1.onValue)(listItemsInDB, function (snapshot) {
    if (snapshot.exists()) {
...
function appendlistItemsEl(item) {
    ...
    deleteButtonEl.addEventListener("click", function (event) {
        let exactLocationOfItemInDB = (0, database_1.ref)(database, `listItems/${itemID}`);
        (0, database_1.remove)(exactLocationOfItemInDB);
    });
    ...
    if (inputElement) {
        ...
        if (selectedItemID) {
            if (newValue.length > 1 && newValue.length <= 15) {
                ...
                let exactLocationOfItemInDB = (0, database_1.ref)(database, `listItems/${selectedItemID}`);
                (0, database_1.set)(exactLocationOfItemInDB, newValue);
